...
# We will CONTINUE form the last part and make the editions for numerical features.
    ...
    if len(attrs) <= 0 or poscount == 0 or negcount == 0:
        ...
        if splitingpoint != -1:
            ...
            for childs in range(0,2):
                ...
                root.children[key] = building_tree(data2,newattr,depth+1)
        else:
...
#             if math.isnan(row[x]) == False:
                ...
                z = predic_missing_node(root,row)
                if z == 0:
                    zerocount += 1
                else:
                    onecount += 1
